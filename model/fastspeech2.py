...
class FastSpeech2(nn.Module):
    ...
    def forward(
        ...
        # print(spker_embed.shape)
        # print(sp_embed.shape)
        # sp_embed = sp_embed.squeeze()

        encoder_output = self.encoder(text_seq, text_mask)
        encoder_output = self.speaker_integrator(encoder_output, sp_embed)
        ...
        variance_adaptor_output = self.speaker_integrator(
            variance_adaptor_output, sp_embed
        )
